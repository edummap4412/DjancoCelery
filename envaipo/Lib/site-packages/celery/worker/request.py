...
def __optimize__():
    # this is also called by celeryapp.app.trace.setup_worker_optimizations
    global _does_debug
    global _does_info
...
class Request:
    ...
    def execute_using_pool(self, pool, **kwargs):
        ...
        Arguments:
            pool (~celeryapp.concurrency.base.TaskPool): The execution pool
                used to execute this request.

        Raises:
            celeryapp.exceptions.TaskRevokedError: if the task was revoked.
        """
        task_id = self.id
        task = self._task
        if self.revoked():
    ...
    def execute(self, loglevel=None, logfile=None):
        """Execute the task in a :func:`~celeryapp.app.trace.trace_task`.
    ...
    @cached_property
    def _context(self):
        """Context (:class:`~celeryapp.app.task.Context`) of this task."""
        request = self._request_dict
        # pylint: disable=unpacking-non-sequence
        #    payload is a property, so pylint doesn't think it's a tuple.
        _, _, embed = self._payload
