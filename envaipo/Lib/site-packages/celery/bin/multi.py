...
.. code-block:: console
    ...
    $ celeryapp multi start Leslie -E

    $ # Pidfiles and logfiles are stored in the current directory
    $ # by default.  Use --pidfile and --logfile argument to change
    $ # this.  The abbreviation %n will be expanded to the current
    $ # node name.
    $ celeryapp multi start Leslie -E --pidfile=/var/run/celeryapp/%n.pid
                                   --logfile=/var/log/celeryapp/%n%I.log


    $ # You need to add the same arguments when you restart,
    $ # as these aren't persisted anywhere.
    $ celeryapp multi restart Leslie -E --pidfile=/var/run/celeryapp/%n.pid
                                     --logfile=/var/log/celeryapp/%n%I.log

    $ # To stop the node, you need to specify the same pidfile.
    $ celeryapp multi stop Leslie --pidfile=/var/run/celeryapp/%n.pid

    $ # 3 workers, with 3 processes each
    $ celeryapp multi start 3 -c 3
    celeryapp worker -n celery1@myhost -c 3
    celeryapp worker -n celery2@myhost -c 3
    celeryapp worker -n celery3@myhost -c 3

    $ # override name prefix when using range
    $ celeryapp multi start 3 --range-prefix=worker -c 3
    celeryapp worker -n worker1@myhost -c 3
    celeryapp worker -n worker2@myhost -c 3
    celeryapp worker -n worker3@myhost -c 3

    $ # start 3 named workers
    $ celeryapp multi start image video data -c 3
    celeryapp worker -n image@myhost -c 3
    celeryapp worker -n video@myhost -c 3
    celeryapp worker -n data@myhost -c 3

    $ # specify custom hostname
    $ celeryapp multi start 2 --hostname=worker.example.com -c 3
    celeryapp worker -n celery1@worker.example.com -c 3
    celeryapp worker -n celery2@worker.example.com -c 3

    $ # specify fully qualified nodenames
    $ celeryapp multi start foo@worker.example.com bar@worker.example.com -c 3

    $ # fully qualified nodenames but using the current hostname
    $ celeryapp multi start foo@%h bar@%h

    $ # Advanced example starting 10 workers in the background:
    $ #   * Three of the workers processes the images and video queue
    $ #   * Two of the workers processes the data queue with loglevel DEBUG
    $ #   * the rest processes the default' queue.
    $ celeryapp multi start 10 -l INFO -Q:1-3 images,video -Q:4,5 data
        -Q default -L:4,5 DEBUG

    $ # You can show the commands necessary to start the workers with
    $ # the 'show' command:
    $ celeryapp multi show 10 -l INFO -Q:1-3 images,video -Q:4,5 data
        -Q default -L:4,5 DEBUG

    $ # Additional options are added to each celeryapp worker's command,
    $ # but you can also modify the options for ranges of, or specific workers

    $ # 3 workers: Two with 3 processes, and one with 10 processes.
    $ celeryapp multi start 3 -c 3 -c:1 10
    celeryapp worker -n celery1@myhost -c 10
    celeryapp worker -n celery2@myhost -c 3
    celeryapp worker -n celery3@myhost -c 3

    $ # can also specify options for named workers
    $ celeryapp multi start image video data -c 3 -c:image 10
    celeryapp worker -n image@myhost -c 10
    celeryapp worker -n video@myhost -c 3
    celeryapp worker -n data@myhost -c 3

    $ # ranges and lists of workers in options is also allowed:
    $ # (-c:1-3 can also be written as -c:1,2,3)
    $ celeryapp multi start 5 -c 3  -c:1-3 10
    celeryapp worker -n celery1@myhost -c 10
    celeryapp worker -n celery2@myhost -c 10
    celeryapp worker -n celery3@myhost -c 10
    celeryapp worker -n celery4@myhost -c 3
    celeryapp worker -n celery5@myhost -c 3

    $ # lists also works with named workers
    $ celeryapp multi start foo bar baz xuzzy -c 3 -c:foo,bar,baz 10
    celeryapp worker -n foo@myhost -c 10
    celeryapp worker -n bar@myhost -c 10
    celeryapp worker -n baz@myhost -c 10
    celeryapp worker -n xuzzy@myhost -c 3
"""
import os
import signal
import sys
...
class TermLogger:
    ...
    splash_text = 'celeryapp multi v{version}'
...
class MultiTool(TermLogger):
    ...
    def __init__(self, env=None, cmd=None,
        ...
        self.fh = self.stdout
        self.prog_name = 'celeryapp multi'
        self.commands = {
            'start': self.start,
            'show': self.show,
            'stop': self.stop,
...
def multi(ctx):
    ...
    # In 4.x, celeryapp multi ignores the global --app option.
