"""The ``celeryapp shell`` program, used to start a REPL."""
...
def shell(ctx, ipython=False, bpython=False,
          ...
          gevent=False):
    """Start shell session with convenient access to celeryapp symbols.

    The following symbols will be added to the main globals:
    - ``celeryapp``:  the current application.
    ...
    if eventlet:
        import_module('celeryapp.concurrency.eventlet')
    if gevent:
        import_module('celeryapp.concurrency.gevent')
    import celery
    app = ctx.obj.app
    ...
    if not without_tasks:
        locals.update({
            ...
            if not task.name.startswith('celeryapp.')
        })
