    ...
    def convert(self, value, param, ctx):
        if ctx.obj.app.IS_WINDOWS and value:
            self.fail('-B option does not work on Windows.  '
                      'Please run celeryapp beat as a separate service.')
...
@click.option('-E',
              ...
              help="Send task-related events that can be captured by monitors"
                   " like celeryapp events, celerymon, and others.")
...
def worker(ctx, hostname=None, pool_cls=None, app=None, uid=None, gid=None,
    ...
    Examples
    --------
    $ celeryapp --app=proj worker -l INFO
    $ celeryapp -A proj worker -l INFO -Q hipri,lopri
    $ celeryapp -A proj worker --concurrency=4
    $ celeryapp -A proj worker --concurrency=1000 -P eventlet
    $ celeryapp worker --autoscale=10,0
    ...
    try:
        ...
        worker = app.Worker(
            ...
            logfile=logfile,  # node format handled by celeryapp.app.log.setup
