...
This module is the 'program-version' of :mod:`celeryapp.worker`.
...
class Worker(WorkController):
    ...
    def on_start(self):
        ...
        if warn_deprecated:
            if app.conf.maybe_warn_deprecated_settings():
                logger.warning(
                    "Please run `celeryapp upgrade settings path/to/settings.py` "
    ...
    def startup_info(self, artlines=True):
        ...
        if not isinstance(app.loader, AppLoader):
            ...
            if loader.startswith('celeryapp.loaders'):  # pragma: no cover
                loader = loader[14:]
            appr += f' ({loader})'
        if self.autoscale:
...
def install_worker_restart_handler(worker, sig='SIGHUP'):
    ...
    def restart_worker_sig_handler(*args):
        ...
        safe_say(f"Restarting celeryapp worker ({' '.join(sys.argv)})")
        import atexit
