...
class Task:
    ...
    __trace__ = None
    __v2_compat__ = False  # set by old base in celeryapp.task.base

    MaxRetriesExceededError = MaxRetriesExceededError
    OperationalError = OperationalError

    #: Execution strategy used, or the qualified name of one.
    Strategy = 'celeryapp.worker.strategy:default'

    #: Request class used, or the qualified name of one.
    Request = 'celeryapp.worker.request:Request'
    ...
    def shadow_name(self, args, kwargs, options):
        ...
        Example:
            .. code-block:: python
                ...
                from celeryapp.utils.imports import qualname
    ...
    def signature(self, args=None, *starargs, **starkwargs):
        ...
        Returns:
            :class:`~celeryapp.signature`:  object for
    ...
    def chunks(self, it, n):
        """Create a :class:`~celeryapp.canvas.chunks` task for this task."""
        from celery import chunks
        return chunks(self.s(), it, n, app=self.app)

    def map(self, it):
        """Create a :class:`~celeryapp.canvas.xmap` task from ``it``."""
        from celery import xmap
        return xmap(self.s(), it, app=self.app)

    def starmap(self, it):
        """Create a :class:`~celeryapp.canvas.xstarmap` task from ``it``."""
    ...
    def replace(self, sig):
        ...
        if isinstance(sig, group):
            ...
            sig |= self.app.tasks['celeryapp.accumulate'].s(index=0)
